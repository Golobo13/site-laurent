...
export default function LandingPage() {
  ...
  useEffect(() => {
    const id = setInterval(() => {
      setPubIndex((i) => (i + 1) % PUBLICATIONS.length);
    }, 10000);
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 text-slate-100 relative">
      ...
      <Section id="conseils" kicker="Publications" title="Laurent">
        <div className="mx-auto max-w-xl">
          {(() => {
            const article = PUBLICATIONS[pubIndex];
            return (
              <a
                key={pubIndex}
                href={article.url}
          ...
          <div className="mt-5 flex items-center justify-center gap-2">
            {PUBLICATIONS.map((_, idx) => (
              <button
                key={idx}
                type="button"
                aria-label={`Voir la publication ${idx + 1}`}
                onClick={() => setPubIndex(idx)}
                className={`h-2 rounded-full transition-all duration-300 ${
                  idx === pubIndex ? "w-6 bg-purple-400" : "w-2 bg-slate-600 hover:bg-slate-500"
                }`}
              />
            ))}
          </div>
